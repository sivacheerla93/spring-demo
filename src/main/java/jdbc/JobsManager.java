...
public class JobsManager implements CommandLineRunner {
	...
	public void showAverageSalary() {
		Double avgSalary = jdbcTemplate.queryForObject("select round(avg(salary)) from employees", Double.class);
	...
	public void insertJob(String jobId, String jobTitle, int minSalary, int maxSalary) {
		int count = jdbcTemplate.update("insert into jobs values(?, ?, ?, ?)", jobId, jobTitle, minSalary, maxSalary);
		if (count == 0) {
			System.out.println("Adding job failed!");
		} else {
	...
	public void run(String... args) {
		...
		// updateMinSalary();
		// sjiDemo.add();
		// insertJob("SD", "Spring Developer", 10000, 40000);
	}
